...
def generate_population_data(num_rows: int) -> pd.DataFrame:
    """
    Generate synthetic population registry data.
    
    Args:
        num_rows: Number of records to generate
    
    Returns:
        DataFrame with population registry data
    """
    data = []
    ...
    for i in range(num_rows):
        ...
        # Generate cell number (South African format)
...
def generate_application_data(num_rows: int, population_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    for i in range(num_rows):
        ...
        if random.random() < 0.95:  # 95% valid references
            ...
            # Get province from population registry
...
def save_datasets(population_df: pd.DataFrame, application_df: pd.DataFrame, output_dir: str = 'data'):
    """
    Save datasets to CSV and XLSX formats.
    ...
    # Save population registry
    population_csv = os.path.join(output_dir, 'population_registry.csv')
    population_xlsx = os.path.join(output_dir, 'population_registry.xlsx')
    population_df.to_csv(population_csv, index=False)
    population_df.to_excel(population_xlsx, index=False, engine='openpyxl')
    
    # Save applications
    application_csv = os.path.join(output_dir, 'dha_applications.csv')
    application_xlsx = os.path.join(output_dir, 'dha_applications.xlsx')
    ...
    print(f"  - {population_csv}")
    print(f"  - {population_xlsx}")
    print(f"  - {application_csv}")
    print(f"  - {application_xlsx}")
...
def main():
    ...
    # Step 1: Generate population registry data
    print("\n[1/4] Generating population registry data...")
    population_df = generate_population_data(POPULATION_ROWS)
    
    # Step 2: Inject data quality issues into population data
    print("[2/4] Injecting data quality issues into population data...")
    population_df, pop_issues = inject_population_quality_issues(
        population_df, DUPLICATE_RATE, MISSING_VALUE_RATE, INVALID_VALUE_RATE
    )
    
    # Step 3: Generate application data
    print("[3/4] Generating DHA application data...")
    application_df = generate_application_data(APPLICATION_ROWS, population_df)
    
    # Step 4: Inject additional data quality issues into application data
    print("[4/4] Injecting data quality issues into application data...")
    ...
    save_datasets(population_df, application_df)
